...
#include "Protocol2.h" // Header file for this source file.
...
} STATES; // States for packet processing state machine

static int PktReady = 0; // Flag indicating packet readiness for processing
 ...
 * @brief Computes the BSD checksum for a given byte.
 * @note Circular rotation is performed, and the byte is added to the checksum.
 */
 ...
 * @brief Initializes the protocol, setting up UART and the state machine.
 * @note UART is configured, packet buffer is initialized, and state machine is reset.
 */
...
uint8_t Protocol_QueuePacket(void); // Unused, handled in build state manually
int Protocol_GetInPacket(uint8_t *type, uint8_t *len, unsigned char *msg); // Unused, part of state machine, handled differently
unsigned char Protocol_ReadNextPacketID(void); // Unused, unclear usage
void flushPacketBuffer(void); // Unused, state changes or packet loss on error

/**
 * @Function Protocol_SendDebugMessage
 * @param Message - Debug message to send
 * @return SUCCESS or ERROR
 * @brief Sends a debug message, including the compilation timestamp.
 * @note Formats the debug message with the compilation date and time.
 */
 ...
 * @note Constructs and sends a packet including the header, tail, and checksum.
 */
 ...
 * @note Swaps the byte order of the input number.
 */
 ...
 * @return SUCCESS or ERROR
 * @brief Checks if the packet buffer is full.
 * @note Returns ERROR if the buffer is full, otherwise SUCCESS.
 */
 ...
 * @brief Builds a received packet from incoming bytes using a state machine.
 * @note Processes incoming bytes into a packet, transitioning through states.
 */
 ...
 * @return SUCCESS or ERROR
 * @brief Parses the received packet and takes action based on the packet ID.
 * @note Handles different message types and performs the corresponding actions.
 */
 ...
 * @note Continuously processes incoming bytes and handles packet parsing.
 */
void runProtocol() {
